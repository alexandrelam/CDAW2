...
<body>
	<div id="app">
		...
		<div class="flex justify-around mt-10 flex-wrap">
							...
							<c:forEach items="${weatherlist}" var="weather">
								<tr>
									<td class="px-6 py-4"><c:out
											value="${weather.countryName}" /></td>
									<td class="px-6 py-4"><c:out
											value="${weather.countryCapital}" /></td>
									<td class="px-6 py-4"><c:out
											value="${weather.countryTemp}" /></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
			...
			<img alt="clock" class="w-96 h-96"
				src="http://localhost:8080/imt.nordeurope.douai.j2ee.tp.AlexandreLam/GraphicWeatherServlet">
		</div>
	</div>
</body>
</html>